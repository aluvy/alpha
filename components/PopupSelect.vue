  ...
  <Transition name="popupNormal">
    <div class="pop_layer normal" :class="{ open : isOpenClass, close : !isOpenClass }" v-if="isOpen">
      ...
      <div class="pop_wrap" ref="popWrap" tabindex="0">
        ...
        <div class="pop_content" v-if=" option.data == 'MVNO' || option.data == 'MVNO2' "><!-- 알뜰폰 통신사 선택 팝업 -->
        ...
        <div class="pop_content" v-else>
          <ul class="select_list" role="listbox">            
            <li role="none" :class="{ on : a.selected }" v-for="(a, i) in data" :key="i">
              <button type="button" role="option" :aria-selected="a.selected" :data-option="a.title" @click="select(a)">{{ a.title }}</button>
            </li>
          </ul>
        </div>

        <button type="button" class="pop_close" @click="close"><span class="blind">팝업 닫기</span></button>
      </div>
      <div @focus="focus" tabindex="0"></div>
    </div>
  </Transition>
...
export default {
  ...
  methods: {
    ...
    open(){
      ...
      setTimeout(()=>{ this.$refs.popWrap.focus() },100);
    },
    close(){
      scroll.scroll();
  ...
  mounted(){
    setTimeout(()=>{
      ...
      if( this.focusBtn == undefined ) this.focusBtn = this.$parent.$parent.$refs[this.option.targetBtn];
      if( this.focusBtn == undefined ) this.focusBtn = document.querySelector(`#${this.option.targetBtn}`);
    }, 10);
  },
  components: {
    MVNOList,
