<template>
  ...
  <div class="tag_list tagList" :class="{ multi : option.multi, block: option.type == 'block' }" v-if="option.type == 'block' || option.type == 'default'" ref="wrap">
    <!-- multi : 다중선택, block : 펼쳐져있음, default : 가로스크롤 -->
    <ul>
      <li role="none" class="other" v-if="getSelectedData.length" ref="selectItem"><!-- selected -->
        <button type="button" aria-haspopup="true" @click="selected.open = !selected.open">{{ selected.title }}</button>
      </li>
      <li role="none" :class="{ on : a.selected }" v-for="(a, i) in getData" :key="i" ref="item">
        <button type="button" role="option" :aria-selected="a.selected" v-bind:disabled="a.disabled" @click="tagClick(i)">{{ a.title }}</button>
      </li>
    </ul>
    <div class="other_pop" :class="{ on : selected.open }" v-if="getSelectedData.length" @click=" selected.open = !selected.open "><!-- selected popup -->
      <ul role="listbox">
        <li role="none" :class="{ on : a.selected }" v-for="(a, i) in getSelectedData" :key="i">
          <button type="button" role="option" :aria-selected="a.open" @click="setSelectedTitle(i)">{{ a.title }}</button>
        </li>
      </ul>
    </div>
  </div>
  ...
  <div class="tag_list tagList variable" :class="{ multi : option.multi, on: expanded }" v-if="option.type == 'expanded'" ref="wrap">
    <!-- multi : 다중선택, variable : 펼침버튼, on: 펼쳐짐 -->
    <button type="button" :aria-expanded="expanded" @click="ExpandedClick"><span class="blind">펼쳐 보기</span></button>
    <ul>
      <li role="none" class="other" v-if="getSelectedData.length" ref="selectItem"><!-- selected -->
        <button type="button" aria-haspopup="true" @click="selected.open = !selected.open">{{ selected.title }}</button>
      </li>
  ...
  <div class="tag_list tagList" :class="{ multi : option.multi, on: expanded, variable: variable }" v-if="option.type == 'expandedResize'" ref="wrap">
...
export default {
  ...
  methods: {
    ...
    ExpandedClick(){
      ...
      let isSelectedItem = [];
      this.getData.forEach( (a, i) => { if( a.selected ) isSelectedItem.push(i) });
      setTimeout(()=>{ this.tagMove( isSelectedItem[0] ) }, 0)
    },
    ...
    hasSelectInit(){
      ...
      let getSelectIdx;
      this.getSelectedData.forEach( (a, i) => { if( a.selected ) getSelectIdx = i });
    ...
    resize(){   // option.type = expandedResize
      const wrap = this.$refs.wrap;
      if( wrap == null ) return;
  ...
  mounted(){
    ...
    // expandedResize
    if( this.option.type == 'expandedResize' ){ this.resize(); window.addEventListener('resize', this.resize); }
  },
  unmounted(){
