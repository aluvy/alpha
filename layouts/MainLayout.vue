<template>
  ...
  <div>
    ...
    <div class="wrap">
      ...
      <NuxtPage ref="MainLayout" :frontData="frontData" :popupItem="popupItem" @setLayout="layout=$event" @openPopup="openPopup($event)" @openAlert="openAlert($event)" />
      <AsideFloat :layout="layout" />
      <MainFooter :layout="layout" />
    </div>

    <Popup :frontData="frontData" :popupItem="popupItem" @openPopup="openPopup($event)" @closePopup="closePopup($event)" @openAlert="openAlert($event)" />
...
export default {
  ...
  methods: {
     ...
     */
    openPopup($event){
      const popupItem = { isOpen: true, id: $event[0], lastFocus: $event[1], idx:$event[2] };
